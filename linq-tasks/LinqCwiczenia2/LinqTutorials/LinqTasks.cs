...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            List<Emp> result = Emps.Where(e => e.Job == "Backend programmer").ToList();
            return result;
        }
        ...
        {
            List<Emp> result = Emps.Where(e => e.Job == "Frontend Programmer" && e.Salary == 1000)
        ...
        {
            IEnumerable<Emp> result = Emps.Where(e => e.Salary == LinqTasks.Task3()).ToList();
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Select(e => new {Nazwisko = e.Ename, Praca = e.Job}).ToList();
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, d => d.Deptno,d2 => d2.Deptno, (emp,dept) => new
            {
                dept.Dname,
                emp.Ename
            });
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(e => new{e.Job}).Select(p => new {Praca = 
                p.Key, LiczbaPracownikow = p.Count()}).ToList();
            return result;
        }
        ...
        {
            Emp result = Emps.Where(e => e.Job == "Frontend programmer").OrderBy(e => e.HireDate).First();
            return result;
        }
        ...
        {
            IEnumerable<object> result = (from emp in Emps select new { emp.Ename, emp.Job, emp.HireDate }).Union(
                from emp in Emps where emp.Ename.Contains("Brak wartości") && emp.Job == null && emp.HireDate == null
